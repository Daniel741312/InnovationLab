...
int getGarbageNamesByImage(const char* fileName,
    ...
    aip::Imageclassify* client =
        new aip::Imageclassify(app_id, api_key, secret_key);
    ...
    /*遍历所有的可能结果（可能性依次递减），保存到传入传出参数std::vector<std::string>*
     * objectNames指向的字符串数组*/
    int result_num = classifyResult["result_num"].asInt();
    for (int i = 0; i < result_num; ++i) {
        (*objectNames)
            .push_back(classifyResult["result"][i]["keyword"].asString());
    }
...
int getGarbageCategoryByNames(std::vector<std::string>* objectNames) {
    ...
    */
    ...
    for (std::vector<std::string>::iterator it = (*objectNames).begin();
         it < (*objectNames).end(); ++it) {
        /*得到了完整的POST请求数据*/
