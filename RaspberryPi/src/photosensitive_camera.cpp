...
int waitForMyPic(void) {
    ...
    if (forkRet == 0) {
        int execlRet = execl("/usr/bin/raspistill", "raspistill", "-o",
                             "garbage.jpg", "-t", "100", nullptr);
        if (execlRet < 0) {
            perror("execl error");
            exit(-1);
        }
        ...
        wait(nullptr);
        std::cout << "Child process exit,he should take a picture named "
                     "'garbage.jpg'\n"
                  << std::endl;
    }
    return 0;
